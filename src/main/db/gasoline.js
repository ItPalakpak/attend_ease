...
export async function saveGasolineEntry(entry) {
  ...
  try {
    ...
    if (entry.id) {
      ...
      await logAudit('GASOLINE_UPDATE', 'gasoline_subsidies', entry.id, { liters, date: entry.date, status })
    } else {
      // Insert
      ...
      await logAudit('GASOLINE_CREATE', 'gasoline_subsidies', res.insertId, { liters, date: entry.date, status })
    }
...
async function matchStaffByName(riderName, connection) {
  ...
  const [rows] = await connection.execute("SELECT id, first_name, last_name FROM staff WHERE employment_status = 'Active'")
...
export async function importGasolineFromExcel(filePath) {
  ...
  try {
    ...
    for (let i = 3; i < rows.length; i++) {
      ...
      if (!staffId) {
        console.warn(`Excel import: Rider '${riderName}' not found in staff database, skipping row ${i}`)
        skipCount++
        continue
      }
    ...
    await connection.commit()
    await logAudit('GASOLINE_EXCEL_IMPORT', 'gasoline_subsidies', null, { file: filePath, imported: importCount })
